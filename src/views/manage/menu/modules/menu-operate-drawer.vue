...
// 模块列表
const moduleList = ref<Api.System.AppList>([]);
const moduleOptions = computed(() =>
  moduleList.value.map(item => ({
    label: $t(`modules.${item.appCode}`),
    value: item.appCode
  }))
);
    ...
    <NDrawerContent :title="drawerTitle" :native-scrollbar="false" closable>
      <NForm ref="formRef" :model="model" :rules="rules">
        <NGrid responsive="screen" item-responsive>
          ...
          <NFormItemGi v-if="!isBtn" :span="24" :label="$t('page.system.menu.moduleAssociation')">
            <NSelect
              v-model:value="model.moduleCodes"
              multiple
