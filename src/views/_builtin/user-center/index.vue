...
function createDefaultProfileModel(): ProfileModel {
  return {
    ...
    sex: String(userInfo.userGender) || '0'
  };
}
...
async function updateProfile() {
  ...
  if (!error) {
    ...
    // 更新本地用户信息
    if (userInfo) {
      userInfo.nickName = profileModel.nickName ?? '';
      userInfo.userEmail = profileModel.email ?? '';
      userInfo.userPhone = profileModel.phonenumber ?? '';
      userInfo.userGender = Number(profileModel.sex);
      ...
      <div class="flex-x-center flex-wrap gap-24px">
        ...
        <NDescriptions :column="1" label-placement="left" label-width="120px">
          ...
          <NDescriptionsItem label="所属部门">
            <div class="text-14px">{{ userInfo.deptName || '未分配' }}</div>
          </NDescriptionsItem>
          <NDescriptionsItem label="所属角色">
            <NSpace>
              <NTag v-for="role in userInfo.roles" :key="role" type="primary" size="small">
                {{ role }}
              </NTag>
            </NSpace>
          </NDescriptionsItem>
